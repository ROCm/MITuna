...
def compile()
{
  ...
  sh "srun --no-kill -p ${partition} -N 1-10 -l bash -c \"docker run ${docker_args} ${tuna_docker_name}"
        " sh -c 'celery -A tuna.celery_app.celery worker -l info -E --detach --logfile=/tmp/tunalogs -n celery_worker"
        " && python3 /tuna/tuna/go_fish.py miopen ${compile_cmd} --session_id ${params.session_id}'\""
}
