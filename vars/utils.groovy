...
def finApplicability(){
    ...
    tuna_docker.inside("--network host  --dns 8.8.8.8") {
        ...
        sh "./tuna/miopen/modules/import_configs.py -t recurrent_${branch_id} --mark_recurrent -f utils/recurrent_cfgs/alexnet_4jobs.txt"
        sh "./tuna/miopen/modules/import_configs.py -t recurrent_${branch_id} --mark_recurrent -f utils/recurrent_cfgs/resnet50_4jobs.txt"

        sh "./tuna/miopen/modules/import_configs.py -t recurrent_${branch_id}_nhwc --mark_recurrent -f utils/configs/conv_configs_NHWC.txt"
        sh "opentelemetry-instrument python3 ./tuna/miopen/modules/import_configs.py -t recurrent_${branch_id}_nchw --mark_recurrent -f utils/configs/conv_configs_NCHW.txt"
        ...
        sh "./tuna/miopen/modules/import_configs.py -t recurrent_${branch_id}_bn --mark_recurrent -f utils/configs/batch_norm.txt -C batch_norm"
...
def finFindCompile(){
    ...
    tuna_docker.inside("--network host  --dns 8.8.8.8 ") {
        ...
        sh "./tuna/miopen/modules/load_job.py -l finFind_${branch_id} --all_configs --fin_steps \"miopen_find_compile, miopen_find_eval\" --session_id ${sesh1} ${job_lim}"
        ...
        //runsql("delete from conv_job;")
        //runsql("alter table conv_job AUTO_INCREMENT=1;")
        sh "./tuna/miopen/modules/load_job.py -l finFind_${branch_id}_nhwc -t recurrent_${branch_id}_nhwc --fin_steps \"miopen_find_compile, miopen_find_eval\" --session_id ${sesh1} ${job_lim}"
        ...
        sh "./tuna/miopen/modules/import_configs.py -t recurrent_${branch_id}_nchw --mark_recurrent -f utils/configs/conv_configs_NCHW.txt"
        def num_cfg_nchw = runsql("SELECT count(*) from conv_config;")
        println "Count(*) conv_config table: ${num_cfg_nchw}"
        sh "./tuna/miopen/modules/load_job.py -l finFind_${branch_id}_nchw -t recurrent_${branch_id}_nchw --fin_steps \"miopen_find_compile, miopen_find_eval\" --session_id ${sesh1} ${job_lim}"
...
def finFindEval(){
    ...
    tuna_docker.inside("--network host  --dns 8.8.8.8 ${docker_args}") {
        ...
        def fdb_file = sh(script: "./tuna/miopen/modules/export_db.py -a ${arch} -n ${num_cu} -f --session_id ${sesh1}", returnStdout: true)
        archiveArtifacts  "${fdb_file}"
        def kdb_file = sh(script: "./tuna/miopen/modules/export_db.py -a ${arch} -n ${num_cu} -k --session_id ${sesh1}", returnStdout: true)
        ...
        def fdb_file_nhwc = sh(script: "./tuna/miopen/modules/export_db.py -a ${arch} -n ${num_cu} -f --session_id ${sesh1} --filename fdb_nhwc", returnStdout: true)
        ...
        def kdb_file_nhwc = sh(script: "./tuna/miopen/modules/export_db.py -a ${arch} -n ${num_cu} -k --session_id ${sesh1} --filename kdb_nhwc", returnStdout: true)
        ...
        def fdb_file_nchw = sh(script: "./tuna/miopen/modules/export_db.py -a ${arch} -n ${num_cu} -f --session_id ${sesh1}", returnStdout: true)
        ...
        def kdb_file_nchw = sh(script: "./tuna/miopen/modules/export_db.py -a ${arch} -n ${num_cu} -k --session_id ${sesh1}", returnStdout: true)
...
def loadJobTest() {
    ...
    tuna_docker.inside("--network host  --dns 8.8.8.8 ${docker_args}") {
        ...
        sh "./tuna/miopen/modules/import_configs.py -t recurrent_${branch_id} --mark_recurrent -f utils/recurrent_cfgs/alexnet_4jobs.txt"
        sh "./tuna/miopen/modules/import_configs.py -t recurrent_${branch_id} --mark_recurrent -f utils/configs/conv_configs_NHWC.txt"
        def out = runsql("SELECT count(*) FROM conv_config_tags WHERE tag='recurrent_${branch_id}' ;")
        assert out.toInteger() > 0

        //reset job table
        runsql("DELETE FROM conv_job;")
        sh "./tuna/miopen/modules/load_job.py -t recurrent_${branch_id} -l recurrent_${branch_id} --session_id ${sesh1} ${job_lim}"
        out = runsql("SELECT count(*) FROM conv_job WHERE reason='recurrent_${branch_id}' and session=${sesh1} ;")
        assert out.toInteger() > 0

        sh "./tuna/miopen/modules/import_configs.py -t batch_norm_test -f utils/configs/batch_norm.txt -C batch_norm"
        ...
        sh "./tuna/miopen/modules/load_job.py -t batch_norm_test -l batch_norm_test -C batch_norm --session_id ${sesh2}"
        ...
        sh "./tuna/miopen/modules/load_job.py -t recurrent_${branch_id} -l recurrent_${branch_id} -s ConvHipImplicitGemmV4R1Fwd --session_id ${sesh1}"
...
def perfCompile() {
    ...
    tuna_docker.inside("--network host --dns 8.8.8.8 ${docker_args} ") {
        ...
        sh "./tuna/miopen/modules/import_configs.py -t alexnet_${branch_id} --mark_recurrent -f utils/recurrent_cfgs/alexnet_4jobs.txt"
        sh "./tuna/miopen/modules/load_job.py -t alexnet_${branch_id} -l alexnet_${branch_id} --session_id ${sesh1} --fin_steps miopen_perf_compile,miopen_perf_eval ${job_lim}"
        ...
        sh "./tuna/miopen/modules/import_configs.py -t conv_${branch_id}_v2 --mark_recurrent -f utils/configs/conv_configs_NHWC.txt"
        sh "./tuna/miopen/modules/import_configs.py -t conv_${branch_id}_v2 --mark_recurrent -f utils/configs/conv_configs_NCHW.txt"
        sh "./tuna/miopen/modules/load_job.py -t conv_${branch_id}_v2 -l conv_${branch_id}_v2 --session_id ${sesh1} --fin_steps miopen_perf_compile,miopen_perf_eval ${job_lim}"
        // Get the number of jobs
...
def perfEval_gfx908() {
    ...
    tuna_docker.inside("--network host --dns 8.8.8.8 ${docker_args} ") {
        ...
        sh "./tuna/miopen/modules/update_golden.py --session_id ${sesh1} --golden_v ${next_gold_v} --base_golden_v ${last_gold_v}"
...
{ 
  ...
  tuna_docker.inside("${docker_run_args}") {
      ...
      echo "/tuna/tuna/miopen/modules/load_job.py --session_id ${params.session_id} ${script_args}"
      sh "python3 /tuna/tuna/miopen/modules/load_job.py --session_id ${params.session_id} ${script_args}"
  }
  tuna_docker.push()
}
