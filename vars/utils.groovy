...
def finFindCompile(){
    ...
    tuna_docker.inside("--network host  --dns 8.8.8.8 ") {
        ...
        sh "opentelemetry-instrument python3 --traces-exporter console ./tuna/go_fish.py --local_machine --fin_steps miopen_find_compile -l finFind_${branch_id} --session_id ${sesh1}"
...
def compile()
{
  ...
  sh "srun --no-kill -p ${slurm_partition} -N 1-10 -l bash -c 'docker run ${docker_args} ${tuna_docker_name} python3 /tuna/tuna/go_fish.py --local_machine ${compile_cmd} -l ${params.job_label} '"
}
