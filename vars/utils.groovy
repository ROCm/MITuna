...
def pytestSuite3() {
    ...
    tuna_docker.inside("--network host  --dns 8.8.8.8") {
        ...
        sshagent (credentials: ['bastion-ssh-key']) {                 
           ...
           sh "wget --inet4-only http://ginger.amd.com:8080/job/Test_mb/job/rk_coverage_auto/lastSuccessfulBuild/artifact/develop_percent_coverage.txt"
...
def runLint() {
    node {
          ...
          tuna_docker.inside("") {
            ...
            sh "cd tuna && mypy miopen/scripts/corrupt_configs.py --ignore-missing-imports --follow-imports=skip"
