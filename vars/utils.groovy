...
def compile()
{
  ...
  sh "srun --no-kill -p ${partition} -N 1-10 -l bash -c 'echo ${env.CREDS_PSW} | sudo docker login -u ${env.CREDS_USR} --password-stdin && hostname'"
  sh "srun --no-kill -p ${partition} -N 1-10 -l bash -c 'hostname'"
