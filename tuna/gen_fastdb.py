...
def check_findDB(findDB, cols_with_conv_params=['fdb_key'], strict=False):
  ...
  if not df_tools.is_col_unique(findDB, col_name='session'):
    if strict:
      raise AssertionError(f'data from multiple tuning sessions present in the FindDB')
    else:
      logging.warning(f'data from multiple tuning sessions present in the FindDB')
      issues.append(MULTIPLE_SESSIONS)

  num_duplicates = findDB.duplicated( subset=cols_with_conv_params+['solver'] ).sum()
  if num_duplicates > 0:
    duplicates = findDB[ findDB.duplicated( subset=cols_with_conv_params+['solver'], keep=False ) ]
    duplicates = duplicates.sort_values( cols_with_conv_params + ['solver', 'kernel_time'] )
    ...
    for i, (_, df) in enumerate(duplicates):
      for j, (row_id, row) in enumerate(df.iterrows()):
        ...
        for colname, val in zip(cols_with_conv_params, row[cols_with_conv_params]):
    ...
    if strict:
      ...
      raise AssertionError(f'{num_duplicates} duplicate entries detected in FindDB')
    else:
...
def describe_fastDB(fastDB):
  if len(fastDB) == 0:
    ...
    logging.info(f'Total entries in FastDB (=unique configs in FindDB): %d' % len(fastDB))
    logging.info(f'Number of unique solvers in FastDB: %d' % len(fastDB['solver'].unique()))


def load_fastDB(fastDB_pickle_filename):
  fastDB = io_tools.safe_load(None, fastDB_pickle_filename, df_tools.from_pickle)
  describe_fastDB(fastDB)
  return fastDB


def findDB_to_nonthresholded_fastDB(findDB, cols_with_conv_params=['fdb_key'], n_fastest=1):
  num_duplicates = findDB.duplicated( subset=cols_with_conv_params+['solver'] ).sum() 
  if num_duplicates > 0:
  ...
  if threshold > 0:
    ...
    for solver in findDB_solver_counts:
      if solver in fastDB_solver_counts:
        ratio_in_fastDB = fastDB_solver_counts[solver] / sum(fastDB_solver_counts.values())
      else:
        ratio_in_fastDB = 0
    ...
    if len(solvers_below_threshold) == 0:
      logging.warning('threshold too loose: none of the solvers are below threshold')
    else:
      if keep_fundamental:
        logging.log('removing non-fundamental solvers below threshold...', end_char='\r')
        solvers_to_remove = filter_out(solvers_below_threshold, is_fundamental_solver)
      else:
      ...
      for solver in findDB_solver_counts:
        ...
        if solver in solvers_to_remove:
          ...
          logging.log('+ ' + string)
      logging.log('solver names with a "-" infront got replaced (where a replacement was available)')
...
if __name__ == '__main__':
  ...
  parser.add_argument('-i', '--in', type=str, default=default_in_filename, dest='input',
      help=f'filename for the pickled FindDB Pandas dataframe (current: {default_in_filename})')
  parser.add_argument('-o', '--out', type=str, default=default_out_dirname, dest='output',
      help=f'dir to output the pickled FastDB Pandas dataframe to (current: {default_out_dirname})')
  parser.add_argument('-t', '--threshold', type=float, default=0, dest='threshold',
      help=f'replace all solvers w/ lesser frequency than the threshold (default: 0)')
  parser.add_argument('--keep_fundamental', action='store_true', default=False, dest='keep_fundamental',
      help=f'specify this flag to keep the fundamental solvers despite them being infrequent')
  ...
  io_tools.safe_save(fastDB, os.path.join(args.output, 'fastDB.pkl'), df_tools.to_pickle)
