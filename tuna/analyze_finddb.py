...
class SolverCountsWriter():
  def __init__(self, choice, sep=","):
    ...
    self.header = self.sep.join(( str(key) for key, val in choice.items() )) + divider + f"solver{self.sep}count"
    self.prefix = self.sep.join(( f"{val}" for key, val in choice.items() )) + divider
...
class StatsEntry(History):
  ...
  def __init__(self, title):
    super().__init__(*StatsEntry.conv_params_names, 'time_diff', 'time_ratio', 'alternate_solver', title=title, track_stats='auto')
    ...
    for j, sj in enumerate(d[si]):
      ...
      if verbosity >= 1:
        if len(d[si][sj]['comprehensive'][0]) == 0:
          logging.warning(f'no stats: solver {sj} is never the next best alternative to {si}. dumping empty files...')
        d[si][sj]['comprehensive'][0].to_csv(filename=os.path.join(out_dir, f'{si}', f'{si}_{sj}.csv')) 
        d[si][sj]['comprehensive'][0].plot(filename=os.path.join(out_dir, f'{si}', f'{si}_{sj}.png'),
                       title=f'{si_name} -> {sj_name}', min_unique_entries=1)

    if len(si_replacement_stats) == 0:
      logging.warning(f'no stats: problems that {si} solves best have no alternate solver for. dumping empty files...')

    si_replacement_stats.to_csv(filename=os.path.join(out_dir, f'{si}.csv'))
    si_replacement_stats.plot(filename=os.path.join(out_dir, f'{si}.png'), 
...
def analyze_explodedFindDB(explodedFindDB, cols_with_conv_params, out_dirname, verbosity=0, solver_counts_writer=dict_to_csv):
  ...
  explodedFastDB = findDB_to_nonthresholded_fastDB(explodedFindDB, cols_with_conv_params)
  ...
  stats_table = { si: { sj: { 'concise': StatsEntry(title='concise'),
                'comprehensive': [StatsEntry(title=i) for i in range(1, len(solver_counts))] 
               }
  ...
  sorted_explodedFindDB = temp.drop_duplicates( cols_with_conv_params+['solver'], keep='first' )
  ...
  sorted_explodedFindDB_numeric = sorted_explodedFindDB.apply( to_numeric_where_possible ) 
  sorted_explodedFindDB_numeric_grouped = sorted_explodedFindDB_numeric.groupby( cols_with_conv_params )

  progressbar = ProgressBar(end_val=len(sorted_explodedFindDB_numeric), title='COLLECTING SLOWDOWN STATS')
  entries_processed = 0
  for _, df in sorted_explodedFindDB_numeric_grouped:
    ...
    for i in range(1, len(df)):
      alternate = df.iloc[i, :]
      stats_entry_concise = stats_table[ fastest['solver'] ][ alternate['solver'] ][ 'concise' ]
      stats_entry_comprehensive = stats_table[ fastest['solver'] ][ alternate['solver'] ][ 'comprehensive' ][ i-1 ] 

      for stats_entry in [stats_entry_concise, stats_entry_comprehensive]:
        stats_entry.add( **conv_params,
                time_diff = alternate['kernel_time'] - fastest['kernel_time'],
                time_ratio = wierd_ratio( alternate['kernel_time'], fastest['kernel_time'] ),
  ...
  io_tools.safe_save(plt, os.path.join(out_dirname, 'solver_histogram.png'), plot_tools.save)
  plt.close('all')
  ...
  plt.xlabel('Solver ID')
  plt.xticks(range(len(y)), labels=x, rotation='vertical', fontsize=7)
  io_tools.safe_save(plt, os.path.join(out_dirname, 'fastest_solver_histogram.png'), plot_tools.save)
  plt.close('all')
  ...
  parse_stats_table(stats_table, os.path.join(out_dirname, 'slowdown_analysis'), verbosity=verbosity)
...
if __name__ == '__main__':
  ...
  explodedFindDB, cols_with_conv_params = to_explodedFindDB(findDB, args.direction, args.layout, args.precision)

  choices = df_tools.unique_combinations( explodedFindDB[['Direction', 'Layout', 'Precision', 
                              'FilterDim0', 'FilterDim1', 'FilterDim2',
  ...
  for choice in choices:
    ...
    describe_findDB(M)
    analyze_explodedFindDB(M, cols_with_conv_params, out_dir, verbosity=args.verbosity, 
               solver_counts_writer=SolverCountsWriter(choice))

    io_tools.safe_save(M, os.path.join(out_dir, 'explodedFindDB.pkl'), df_tools.to_pickle)
