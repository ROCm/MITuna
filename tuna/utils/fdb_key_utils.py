...
_2D_TO_3D_FDB_KEY = Mold(from_ptrn=_2D_FDB_KEY_PTRN, to_ptrn=_3D_FDB_KEY_PTRN,
                         from_sep='-', to_sep='-', filler=1)
...
def get_compulsory_2D_fdb_keynames():
    return list_replace(_NAMES_OF_2D_FDB_KEYS, 'DirectionAndUnderscoreSeperatedOptionals', 'Direction')

def get_compulsory_3D_fdb_keynames():
    return list_replace(_NAMES_OF_3D_FDB_KEYS, 'DirectionAndUnderscoreSeperatedOptionals', 'Direction')
...
def explode_fdb_keys(fdb_keys: pd.Series):
    ...
    if 'GroupSize' in conv_params:
        ...
        conv_params['GroupSize'] = map_series(conv_params['GroupSize'], fix_group_size)

    # explode tensor descriptors: "23x43x14" -> 23, 43, 14
    def explode_tensor_descriptors(tensor_descriptors):
        tensor_descriptors_3D = map_series(tensor_descriptors, to_3D_tensor_descriptor)
    ...
    for colname in conv_params:
        if colname in cols_with_tensor_descriptors:
            logging.reset_line()
            logging.log(f'exploding tensor descriptors in {colname}...', end_char='\r')
            exploded_conv_params.append( explode_tensor_descriptors(conv_params[colname]) )
        else:
    ...
    logging.reset_line()
    logging.success(f'fdb keys processed and exploded into convolution parameters')
