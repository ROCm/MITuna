...
def renumber_cols(df, start=0):
    ...
    dfA = dfA.rename(columns = {old_col_num: new_col_num
                                for old_col_num, new_col_num in zip(df, range(start, end))})

    if first_duplicate_index == len(df.columns):
        return dfA
    else:
        dfB = renumber_cols(df.iloc[:, first_duplicate_index:], start=len(dfA.columns))
        return pd.concat([dfA, dfB], axis=1)

def delete_redundant_cols(df, masked_cols=[], min_num_unique_entries=2, inplace=False):
    dropped_cols = []
    for i, col in enumerate(df):
        ...
        if len(unique_entries) < min_num_unique_entries:
            ...
            if inplace:
                ...
                logging.warning('column %s dropped from dataframe: it had just %d unique entr%s' %
                             (col, len(unique_entries), 'y' if min_num_unique_entries == 2 else 'ies'))
            else:
                df = drop_col(df, col, inplace=False)
                dropped_cols.append(col)
