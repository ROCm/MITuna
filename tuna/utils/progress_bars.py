...
class ProgressBar():
  ...
  def __init__(self, end_val, title='Progress', bar_length=10, char_at_end='\n'):
    self.title = title
  ...
  def display(self, progress=0):
    ...
    sys.stdout.write("\r{0}: [{1}] {2}%".format(
      self.title, hashes + spaces, int(round(percent * 100))))
    sys.stdout.flush()
...
def cooldown_timer(cooldown_time, title=None, bar_length=10, char_at_end='\r\033[K', update_rate=1):
  if cooldown_time != 0:
    if title is None:
